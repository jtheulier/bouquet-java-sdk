...
public class ModelApi extends BaseApi {
   ...
   * @param jobId
   * @param ext
   * @param timeout response timeout in milliseconds in case the job is not yet computed. If no timeout set, the
   *        method will return according to current job status.
   * @param maxResults paging size
   * @param startIndex paging start index
   * @param lazy if true, get the analysis only if algety in cache
  ...
  /**
   * Start the reset-password process. Create a new &#39;reset_pwd&#39; AccessToken for the user having the passed
   * email address.
   *
   * @param customerId
   * @param clientId
   ...
   *        http://api.squisolutions.com/release/api/reset_email?access_token={access_token}). The url must
   *        match the Client authorized urls
   * @return String
   * @throws ApiException if fails to make API call
   */
