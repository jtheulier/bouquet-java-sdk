...
public class ClientEngine {
  ...
  public Object getAnalysisJobResults(ModelApi api, ProjectAnalysisJob analysis, Integer timeout, Integer maxResults,
      Integer startIndex, Boolean lazy, String format, String compression, int maxLoops) throws ApiException {
    boolean done = false;
    int loops = 0;
    ...
    if (analysis != null) {
      ...
      while (!done) {
        ...
        result =
            api.getAnalysisJobResults(projectId, jobId, timeout, maxResults, startIndex, lazy, format, compression);
        ProjectAnalysisJob analysisJob = api.getAnalysisJob(projectId, jobId, null, 10000);
        loops++;
        if (result != null || (analysisJob != null && analysisJob.getStatus() != null
            && analysisJob.getStatus() == ProjectAnalysisJob.StatusEnum.DONE)) {
          done = true;
          if (result instanceof ApiException) {
            if (((ApiException) result).getMessage() == "COMPUTING_IN_PROGRESS") {
              result = api.getAnalysisJobResults(projectId, jobId, timeout, maxResults, startIndex, lazy, format,
                compression);
            }
          }
          return result;
  ...
  public boolean executeFacetJob(ModelApi api, Domain domain, List<Dimension> dimensions, boolean wait)
    ...
    if (job.getError() == null) {
      for (Facet facet : remainingFacets) {
        ...
        while (retry > 0) {
          try {
            Facet computingFacet =
                api.getFacet(domain.getId().getProjectId(), job.getId().getFacetJobId(), facet.getId(), null,
                  10, 10, 0);
            if (computingFacet.isProxy() != null && computingFacet.isProxy()) {
              logger.debug("Proxied facet '" + facetName + "'");
              computingFacets.remove(facet);
            } else if (computingFacet.isError() != null && computingFacet.isError() == false
                && (computingFacet.isDone() == true)) {
              if (computingFacet.getDimension() != null
                  && computingFacet.getDimension().getType() == TypeEnum.CATEGORICAL) {
                logger.debug("Facet '" + facetName + "' done");
              }
              computingFacets.remove(facet);
            } else if (computingFacet.isDone() == true && computingFacet.isHasMore() == false
                && computingFacet.isError() == true) {
              logger.debug("Facet '" + facetName + "' in error " + computingFacet.getErrorMessage());
              computingFacets.remove(facet);
            } else if (retry == 1) {
            ...
            if (ae.getApiError() == ApiError.COMPUTING_IN_PROGRESS) {
              ...
              logger.debug("Error occured while checking facet '" + facetName + "' :"
                  + (ae.getMessage() != null ? ae.getMessage()
                      : (ae.getApiError() != null ? ae.getApiError().toString() : "")));
              throw ae;
            }
          }
          retry--;
          if (computingFacets.contains(facet) == false) {
            retry = 0;
          } else {
            synchronized (this) {
              this.wait(5 * 1000L);
    ...
    if (job.getStatus() != ProjectFacetJob.StatusEnum.DONE) {
      ...
      logger.debug(
        "Leaving facet job processing on domain " + facetsDomain.getName() + " with " + facetsNr + " still running");
    }

    return true;
  }

  public boolean executeFacetJob2(ModelApi api, Domain domain, List<Dimension> dimensions, boolean wait)
      throws ApiException, InterruptedException {
    ProjectFacetJob job = new ProjectFacetJob();
    job.setDomains(Arrays.asList(domain.getId()));
    ProjectFacetJobPK facetJobId = new ProjectFacetJobPK();
        ...
        if (job.getStatus() == ProjectFacetJob.StatusEnum.DONE) {
          ...
          if (job.getError() == null) {
            ...
            for (Facet facet : remainingFacets) {
              String facetName = facet.getName();
              try {
                facet = api.getFacet(domain.getId().getProjectId(), job.getId().getFacetJobId(), facet.getId(), null,
                  10, 10, 0);
  ...
  public ProjectAnalysisJob buildAnalysisFromBookmark(ModelApi api, Bookmark bookmark, boolean isSummary,
    ...
    if (bookmark != null) {
      ...
      if ((selection.getCompareTo() == null || selection.getCompareTo().size() == 0)
          && defaultComparePeriod != ComparePeriod.__NONE && bookmark.getConfig().getPeriod() != null) {
        if (bookmark.getConfig().getPeriod().any() != null && bookmark.getConfig().getPeriod().any().size() == 1) {
          ...
          Dimension dim = api.getDimension(bookmark.getId().getProjectId(), bookmark.getConfig().getDomain(),
            APIUtils.splitDomainObject(bookmark.getConfig().getPeriod().any().values().iterator().next())[1], true);
      ...
      List<Metric> allMetrics = api.getMetrics(bookmark.getId().getProjectId(), domain.getDomainId());
      if (isSummary)

      {
        ...
        if (mets != null) {
          for (String met : mets) {
            if (met != null) {
              Metric metric = null;
              MetricPK metPk = new MetricPK();
              metPk.setProjectId(bookmark.getId().getProjectId());
              metPk.setDomainId(domain.getDomainId());
              metPk.setMetricId(met);

              for (Metric allMetric : allMetrics) {
                if (allMetric.getId().getProjectId().equals(metPk.getProjectId())
                    && allMetric.getId().getDomainId().equals(metPk.getDomainId())
                    && allMetric.getId().getMetricId().equals(metPk.getMetricId())) {
                  metric = allMetric;
                }
              }
              if (metric == null) {
            ...
            for (ChosenMetric choosen : choosens) {
              if (met.equals(choosen.getId())) {
                if (choosen.getId() != null && choosen.getId().indexOf("@") == -1) {
                  Metric metric =
                      api.getMetric(bookmark.getId().getProjectId(), domain.getDomainId(), choosen.getId(), false);
                  if (metric == null) {
                    throw new ApiException("Invalid metric Id " + met);
                  }
                  metric.getAccessRights().clear();
