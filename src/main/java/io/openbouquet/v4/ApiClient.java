...
import io.openbouquet.v4.auth.ApiKeyAuth;
import io.openbouquet.v4.auth.Authentication;
import io.openbouquet.v4.auth.HttpBasicAuth;
import io.openbouquet.v4.auth.JWTAuth;
import io.openbouquet.v4.auth.OAuth;
...
public class ApiClient {
	...
	private String basePath = "https://localhost/staging/v4.2";
	...
	public ApiClient() {
		...
		httpClient.setConnectTimeout(0, TimeUnit.SECONDS);
		...
		setUserAgent("OpenBouquet Java API 4.2");

		// Setup authentications (key: authentication name, value: authentication).
		authentications = new HashMap<String, Authentication>();
		authentications.put("kraken_auth", new OAuth());
		authentications.put("jwt_auth", new JWTAuth());
		authentications.put("basic_auth", new HttpBasicAuth());
		authentications.put("api_key_auth", new ApiKeyAuth());
	 ...
	 * Set base path
	 *
	 * @param basePath Base path of the URL (e.g https://localhost/staging/v4.2
	 ...
	 */
	public void setApiKey(String jwt, String clientId) {
		for (Authentication auth : authentications.values()) {
			if (auth instanceof ApiKeyAuth) {
				((ApiKeyAuth) auth).setJwt(jwt);
				((ApiKeyAuth) auth).setClientId(clientId);
				return;
			}
		}
	...
	/**
	 * Helper method to set Client key for login basic loging/pwd authentication.
	 *
	 * @param apiKey API key
	 */
	public void setBasicAuth(LoginConfiguration configuration) {
		for (Authentication auth : authentications.values()) {
			if (auth instanceof HttpBasicAuth) {
				((HttpBasicAuth) auth).setClientId(configuration.getClientId());
				((HttpBasicAuth) auth).setCustomerId(configuration.getCustomerId());
				((HttpBasicAuth) auth).setUsername(configuration.getLogin());
				((HttpBasicAuth) auth).setPassword(configuration.getPassword());
				return;
			}
		}
		throw new RuntimeException("No Client authentication configured!");
	}

	/**
	 * Helper method to set Client key for login using JWT.
	 *
	 * @param apiKey API key
	 */
	public void setJWTKey(JWTConfiguration configuration) {
		for (Authentication auth : authentications.values()) {
			if (auth instanceof JWTAuth) {
				((JWTAuth) auth).setClientId(configuration.getClientId());
				((JWTAuth) auth).setCustomerId(configuration.getCustomerId());
				((JWTAuth) auth).setUserId(configuration.getUserId());
				try {
					((JWTAuth) auth).setPrivateKey(configuration.getPrivateKey());
				} catch (Exception e) {
	...
	public <T> T callApi(Method method, String localVarPath, T localVarPostBody) throws ApiException {
		// query params
		...
		Call call = buildCall(localVarPath, method.toString(), localVarQueryParams, localVarPostBody, localVarHeaderParams, localVarFormParams, localVarAuthNames, null);
		ApiResponse<T> resp = execute(call, localVarReturnType);
		return resp.getData();
	}
	...
	public static  ApiClient buildClient(String basePath) {
		boolean https = basePath.toLowerCase().startsWith("https");
		if (https) {
			try {
				SSLContext sslcontext = SSLContext.getInstance("TLS");
				sslcontext.init(null, new TrustManager[] { APIUtils.getTrustManager() }, null);
