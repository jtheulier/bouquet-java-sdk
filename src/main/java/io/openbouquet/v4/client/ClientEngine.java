 ...
 *******************************************************************************/
package io.openbouquet.v4.client;
...
import io.openbouquet.v4.client.CacheConfiguration.ComparePeriod;
import io.openbouquet.v4.model.Bookmark;
import io.openbouquet.v4.model.ChoosenMetric;
import io.openbouquet.v4.model.Dimension;
import io.openbouquet.v4.model.Dimension.TypeEnum;
import io.openbouquet.v4.model.Domain;
import io.openbouquet.v4.model.DomainPK;
import io.openbouquet.v4.model.Expression;
import io.openbouquet.v4.model.Facet;
import io.openbouquet.v4.model.FacetMember;
import io.openbouquet.v4.model.FacetMemberInterval;
import io.openbouquet.v4.model.FacetSelection;
import io.openbouquet.v4.model.Metric;
import io.openbouquet.v4.model.OrderBy;
import io.openbouquet.v4.model.OrderBy.DirectionEnum;
import io.openbouquet.v4.model.ProjectAnalysisJob;
import io.openbouquet.v4.model.ProjectAnalysisJobPK;
import io.openbouquet.v4.model.ProjectFacetJob;
import io.openbouquet.v4.model.ProjectFacetJobPK;
import io.openbouquet.v4.model.RollUp;
...
public class ClientEngine {
	...
	public Object getAnalysisJobResults(ModelApi api, ProjectAnalysisJob analysis, Integer timeout, Integer maxResults, Integer startIndex, Boolean lazy, String format, String compression,
	...
	protected boolean executeFacetJob(ModelApi api, Domain domain, List<Dimension> dimensions, boolean wait) throws ApiException, InterruptedException {
		...
		if (wait) {
			...
			while (retry > 0) {
				// job = api.getOrStoreFacetJob(domain.getProjectId(), job.getId().getFacetJobId(), null, null);
					...
					if (job.getError() == null) {
						...
						for (Facet facet : remainingFacets) {
							...
							try {
								...
								logger.debug("Error occured while checking facet '" + facetName + "' :" + ae.getMessage() + " with body " + ae.getResponseBody());
								throw ae;
							}
						}
		...
		if (bookmark != null) {
			...
			if ((selection.getCompareTo() == null || selection.getCompareTo().size() == 0) && defaultComparePeriod != ComparePeriod.__NONE && bookmark.getConfig().getPeriod() != null) {
				if (bookmark.getConfig().getPeriod().any().size() == 1) {
					...
					Dimension dim = api.getDimension(bookmark.getId().getProjectId(), bookmark.getConfig().getDomain(), APIUtils.splitDomainObject(bookmark.getConfig().getPeriod().any().values().iterator()
							.next())[1], true);
				...
				if (mets != null) {
					for (ChoosenMetric met : mets) {
						if (met.getId() != null) {
							...
							metric.setExpression(met.getExpression());
