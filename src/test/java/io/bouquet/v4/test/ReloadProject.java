...
public class ReloadProject extends ClientEngine {
	...
	public static void main(String[] args) {
		...
		if (args.length >= 3) {
			...
			try {
				ds.run(args[0], clientConfiguration, projectConfiguration,
						cacheConfiguration);
	...
	public void run(String baseUrl, LoginConfiguration clientConfiguration,
		...
		Project project = api.getProject(projectConfiguration.getProjectId(),
				true);
		// logger.debug("Start refresh for project");

		Bookmark bookmark = api.getBookmark(projectConfiguration.getProjectId(),
				"570e095ec942c00417033e08", true);
		api.getDomain(projectConfiguration.getProjectId(),
				bookmark.getConfig().getDomain(), true);

		ProjectAnalysisJob analysis = buildAnalysisFromBookmark(api, bookmark,
				true, cacheConfiguration.getDefaultComparePeriod());
		analysis.setLimit(new Long(1000));
		analysis = api.putAnalysisJob(analysis.getId().getProjectId(), analysis,
				10000, 100, 1, true, null, null);
		getAnalysisJobResults(api, analysis, 10000, 1000, 1, false, null, null,
				3);
			...
			if (bookmark.getPath() != null
				...
				try {
					if (refreshDomains && refreshedDomains.contains(
						...
						api.getDomain(bookmark.getId().getProjectId(),
								bookmark.getConfig().getDomain(), true);
					...
					hasError = true;
					logger.debug("Refresh domain " + bookmark.getPath() + "/"
							+ bookmark.getConfig().getDomain() + " has errors: "
							+ ae.getMessage(), ae);
				}
			}
		}
