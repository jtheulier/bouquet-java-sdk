...
public class ReloadProject extends ClientEngine {
  ...
  public void run(String baseUrl, LoginConfiguration clientConfiguration, ProjectConfiguration projectConfiguration,
    ...
    ProjectAnalysisJob analysis =
        buildAnalysisFromBookmark(api, bookmark, true, cacheConfiguration.getDefaultComparePeriod());
    analysis.setLimit(Long.valueOf(1000));
    ...
    for (Bookmark bookmark : project.getBookmarks()) {
      if (bookmark.getPath() != null && bookmark.getPath().startsWith("/SHARED") && bookmark.getConfig() != null) {
        try {
          ...
          hasError = true;
          logger.debug("Refresh domain " + bookmark.getPath() + "/" + bookmark.getConfig().getDomain() + " has errors: "
              + ae.getMessage(), ae);
        }
      }
    }
